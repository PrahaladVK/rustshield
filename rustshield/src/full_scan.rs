...
pub fn scan_paths(
...

    {
        ...
        p.files_scanned = 0;
        p.threats_found = 0;
        p.total_roots = total_roots;
        p.current_root_index = 0;
        p.current_file = String::new();
        p.current_root = String::new();
    }
...
pub fn scan_running_processes(
    ...
    for path_str in &exe_paths {
        ...
        if files_scanned % 10 == 0 {
            ...
            files_scanned = 0;
            threats_found = 0; // avoid double-counting
        }
    }
...
fn finish_progress(
    ...
    if was_cancelled {
        log::info!(
            "Scan cancelled — {} files, {} threat(s) ({:.1}s)",
            files_scanned,
            threats_found,
            elapsed.as_secs_f32()
        );
        ...
        log::info!(
            "Scan complete — {} files, no threats ({:.1}s)",
            files_scanned,
            elapsed.as_secs_f32()
        );
    } else {
        log::warn!(
            "Scan complete — {} files, {} THREAT(S) quarantined ({:.1}s)",
            files_scanned,
            threats_found,
            elapsed.as_secs_f32()
        );
