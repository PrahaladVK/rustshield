...
async fn scan_handler(
    ...
    let (roots, label, include_processes) = match req.path.as_str() {
        ...
        "__QUICK__" => {
            ...
            log::info!(
                "Quick scan → {} locations + running processes",
                all_paths.len()
            );
            (
                all_paths,
                "Quick scan (folders + running processes)".to_string(),
                true,
            )
        }
    ...
    let mut summary =
        tokio::task::spawn_blocking(move || scan_paths(&roots_clone, db1, yara1, prog1, cancel1))
            .await
            .unwrap();

    // ── Phase 2: running process scan (Quick scan only) ─────────────
    if include_processes && !s.cancel.load(Ordering::Relaxed) {
        let (db2, yara2, prog2, cancel2) = (
            s.db.clone(),
            s.yara.clone(),
            s.progress.clone(),
            s.cancel.clone(),
...
async fn detections_handler(
    ...
    let items =
        s.db.get_detections(q.page, pp)
            .unwrap_or_default()
            .into_iter()
            .map(|(fp, h, v, ts)| Detection {
                file_path: fp,
                sha256: h,
                verdict: v,
                timestamp: ts,
        ...
        Ok(original_path) => {
            if req.add_exception {
                if let Some(ref it) = item {
                    ...
                    let _ = s.db.add_exception(
                        Some(&it.sha256),
                        Some(&it.original_path),
                        &fname,
                        Some("User restored from quarantine"),
                    );
                }
            }
            Json(RestoreResponse {
                success: true,
                message: "File restored successfully".into(),
                original_path,
            })
        }
...
async fn add_exception_handler(
    State(s): State<AppState>,
...
async fn remove_exception_handler(
    State(s): State<AppState>,
    Path(id): Path<i64>,
) -> Json<SimpleOk> {
...
pub fn build_router(state: AppState) -> Router {
    ...
    Router::new()
        ...
        .route(
            "/exceptions",
            get(exceptions_list_handler).post(add_exception_handler),
        )
