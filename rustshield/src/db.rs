...
impl SignatureDb {
    ...
    pub fn add_signature(&self, sha256: &str, name: &str, sev: i32) -> Result<()> {
        ...
        c.execute(
            "INSERT OR REPLACE INTO signatures (sha256,threat_name,severity) VALUES(?1,?2,?3)",
    ...
    pub fn log_scan(&self, file_path: &str, sha256: &str, verdict: &str) -> Result<()> {
        let c = self.conn.lock().unwrap();
        c.execute(
            "INSERT INTO scan_log (file_path,sha256,verdict) VALUES(?1,?2,?3)",
    ...
    pub fn get_detections(
        ...
        s.query_map(params![per_page, page * per_page], |r| {
            Ok((r.get(0)?, r.get(1)?, r.get(2)?, r.get(3)?))
        })
        .and_then(|r| r.collect())
    }
    pub fn count_detections(&self) -> Result<i64> {
        let c = self.conn.lock().unwrap();
        Ok(c.query_row(
            "SELECT COUNT(*) FROM scan_log WHERE verdict!='clean'",
            [],
            |r| r.get(0),
        )?)
    }
    ...
    pub fn get_quarantine_items(&self) -> Result<Vec<QuarantineItem>> {
        ...
        s.query_map([], |r| {
            Ok(QuarantineItem {
                ...
                quarantine_path: r.get(4)?,
                quarantined_at: r.get(5)?,
            })
        })
        .and_then(|r| r.collect())
    }
    ...
    pub fn mark_restored(&self, id: i64) -> Result<()> {
        let c = self.conn.lock().unwrap();
        c.execute(
            "UPDATE quarantine_log SET restored=1 WHERE id=?1",
            params![id],
        )?;
        Ok(())
    }
    ...
    pub fn add_exception(
        ...
        let c = self.conn.lock().unwrap();
        c.execute(
            "INSERT INTO exceptions (sha256,file_path,file_name,reason) VALUES(?1,?2,?3,?4)",
    ...
    pub fn get_exceptions(&self) -> Result<Vec<ExceptionItem>> {
        let c = self.conn.lock().unwrap();
        let mut s = c.prepare(
            "SELECT id,sha256,file_path,file_name,reason,added_at FROM exceptions ORDER BY id DESC",
        )?;
        s.query_map([], |r| {
            Ok(ExceptionItem {
                id: r.get(0)?,
