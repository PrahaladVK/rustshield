...
fn parse_pe_sections(data: &[u8]) -> Vec<PeSection> {
    ...
    for i in 0..num {
        ...
        if raw_size == 0 || raw_off == 0 || raw_off >= data.len() {
            continue;
        }
        let end = (raw_off + raw_size).min(data.len());
        out.push(PeSection {
            name,
            entropy: shannon_entropy(&data[raw_off..end]),
...
fn run_heuristics(sections: &[PeSection]) -> (u8, Vec<String>) {
    ...
    let pk: Vec<&str> = sections
        .iter()
        .filter_map(|s| {
            PACKER_SECTION_NAMES
                .iter()
                .find(|&&p| s.name.eq_ignore_ascii_case(p))
                .map(|_| s.name.as_str())
        })
        .collect();
    if !pk.is_empty() {
        reasons.push(format!("Known packer section names: {}", pk.join(",")));
        score += 1;
    }
    let non_empty = sections.iter().filter(|s| s.entropy > 0.5).count();
    if non_empty <= 1 && sections.len() <= 2 {
        reasons.push("Very few PE sections — possibly a packed stub".into());
        score += 1;
    }
    (score, reasons)
}
...
fn heuristic_verdict(path: &Path) -> Verdict {
    let ext = path
        ...
        .unwrap_or_default();
    if !PE_EXTS.contains(&ext.as_str()) {
        return Verdict::Clean;
    }
    let pu = path.to_string_lossy().to_uppercase();
    if TRUSTED_PREFIXES.iter().any(|p| pu.starts_with(*p)) {
        return Verdict::Clean;
    }
    let size = path.metadata().map(|m| m.len()).unwrap_or(0) as usize;
    if size < MIN_PE_BYTES || size > MAX_PE_BYTES {
        return Verdict::Clean;
    }
    let data = match fs::read(path) {
        Ok(d) => d,
        Err(_) => return Verdict::Clean,
    };
    ...
    if score >= 2 {
        Verdict::Suspicious(reasons.join(" | "))
    } else {
        Verdict::Clean
    }
}
...
pub fn scan_file(
    ...
    if let Ok(Some((threat_name, _))) = db.lookup(&sha256) {
        return Ok(ScanResult {
            path: path_str,
            sha256,
            verdict: Verdict::Malicious(threat_name),
        });
    }
    ...
    if !hits.is_empty() {
        return Ok(ScanResult {
            path: path_str,
            sha256,
            verdict: Verdict::Malicious(format!("YARA:{}", hits.join(","))),
        });
    }
    ...
    Ok(ScanResult {
        path: path_str,
        sha256,
        verdict: heuristic_verdict(path),
    })
}
